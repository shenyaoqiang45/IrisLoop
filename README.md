...
- ✅ 投射 + 摄像头同步采集联动（闭环第一步）
- ✅ 设备素材拉取（WiFi HTTP）
- ⚠️ BLE 推图：文件传输服务的开始/结束帧格式协议未明确，`push_image.py` 中 start/end payload 暂为占位
- ❌ AI 视觉分析闭环（规划：多模态模型评估投影效果 → 迭代优化内容）

## 路线图

1. **投射** —— BLE 下发自定义图片 / 播放素材组（已完成基础能力）
2. **观察** —— 摄像头采集实际投影效果，量化分析（已完成）
3. **理解** —— 接入多模态 AI 评估差距（图案完整性、朝向、畸变、亮度、材质表现）
4. **优化** —— 自动生成修正后的投影内容，形成数据飞轮
